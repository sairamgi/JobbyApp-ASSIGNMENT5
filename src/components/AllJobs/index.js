...
class AllJobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput} = this.state
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&minimum_package=${radioInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onGetJobsFailureView = () => (
    <div className="failure-img-button-container">
      ...
      <h1 className="failure-heading">Oops! Something wrong</h1>
      <p className="failure-paragraph">We cannot seem to find the page</p>
